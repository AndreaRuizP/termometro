...
static uint8_t SD_SPI_Transfer(uint8_t data){
    uint8_t i, result = 0;
    for(i = 0; i < 8; i++){
        PORTBbits.RB3 = (data & 0x80) ? 1 : 0;
        data <<= 1;
        PORTBbits.RB4 = 1;
        _delay((unsigned long)((2)*(20000000/4000000.0)));
        if(PORTBbits.RB2) result |= (0x80 >> i);
        PORTBbits.RB4 = 0;
        _delay((unsigned long)((2)*(20000000/4000000.0)));
    }
    return result;
...
uint8_t SD_Init(void){
    ...
    for(retry = 0; retry < 200; retry++){
        if(SD_Cmd(0x00, 0) == 0x01) break;
        _delay((unsigned long)((10)*(20000000/4000.0)));
    }
    if(retry >= 200){ PORTBbits.RB5 = 1; return 0; }
    r = SD_Cmd(0x08, 0x000001AA);
    if(r == 0x01){
        for(i = 0; i < 4; i++) SD_SPI_Transfer(0xFF);
    }
    for(retry = 0; retry < 1000; retry++){
        SD_Cmd(0x37, 0);
        if(SD_Cmd(0x29, 0x40000000) == 0x00) break;
        _delay((unsigned long)((10)*(20000000/4000.0)));
    }
    if(retry >= 1000){ PORTBbits.RB5 = 1; return 0; }
    if(SD_Cmd(0x10, 512) != 0x00){ PORTBbits.RB5 = 1; return 0; }
    PORTBbits.RB5 = 1;
    SD_SPI_Transfer(0xFF);
    current_sector = 100;
    current_offset = 0;
...
void SD_AppendText(const char* text){
    ...
    resp = SD_Cmd(0x18, current_sector);
    if(resp != 0x00){ PORTBbits.RB5 = 1; return; }
    ...
    if((resp & 0x1F) != 0x05){ PORTBbits.RB5 = 1; return; }
